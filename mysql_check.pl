...
my $dsn = "DBI:mysql:mysql;host=$mysql";
#my $dsn = "DBI:mysql:mysql;mysql_socket=/data/mysql/mysql.sock";
my $zabbix_sender = '/usr/bin/zabbix_sender';
#my $agent_config = '/etc/zabbix/zabbix_agentd.conf';

my $tmp_file = "/tmp/zbx_mysql.status.$host";
my $zbx_sender_file = "/tmp/zbx_mysql.sender.$host";
my $max_updated = 10;
...
sub get_variables($$) {
    ...
    my $query_variables = 'SHOW GLOBAL VARIABLES where variable_name not like "ft_boolean_syntax"';
...
#    my $result = `$zabbix_sender -c $agent_config -i $zbx_sender_file`;
     my $result = `$zabbix_sender -z 127.0.0.1 --host "$host" -i $zbx_sender_file`;
...
'Handler_update' => 0,                                                                                                                
'Handler_write' => 0,                                                                                                                 
'Innodb_dictionary_cache_memory' => 0,
...
'Innodb_ibuf_cell_count' => 0,
'Innodb_ibuf_used_cells' => 0,
'Innodb_ibuf_inserts' => 0,
